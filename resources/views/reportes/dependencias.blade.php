...
<div class="box-body col-md-12">
    <center>
        <h2 style="color: #4e555b">Total de Dependencais</h2>
    </center>
    <div class="datagrid"><table>
            <thead><tr><th>id</th><th>Nombre</th><th>Codigo</th><th>Fecha de creación</th></tr></thead>
            <tbody>
            @foreach($data as $post)
                <tr class="alt"><td>{{$post['id']}}</td><td>{{$post['nombreDependencias']}}</td><td>{{$post['codigoDependencias']}}</td><td>{{$post['created_at']}}</td></tr>
            @endforeach
            </tbody>
        </table></div>
</div>
