...
<div class="box-body col-md-12">
    <center>
        <h2 style="color: #4e555b">Total de Subseries</h2>
    </center>
    <div class="datagrid"><table>
            <thead><tr><th>Serie</th><th>Nombre</th><th>Codigo</th><th>Fecha de creación</th></tr></thead>
            <tbody>
            @foreach($data as $post)
                <tr class="alt"><td>{{$post->nombreSeries}}</td><td>{{$post->nombreSubSeries}}</td><td>{{$post->codigoSubSeries}}</td><td>{{$post->created_at}}</td></tr>
            @endforeach
            </tbody>
        </table></div>
</div>
